...
function renderizarResumo(): void {
  ...
  for (const cartao of cartoes) {
    ...
    elemento.innerHTML = `
      <span class="valor">${cartao.valor}</span>
      <span class="rotulo">${escaparHtml(cartao.rotulo)}</span>
    `
...
function criarLinhaItem(valores?: { id?: number; produto: string; quantidade: number; valorUnitario: number }): void {
  ...
  for (const campo of [quantidade, valorUnitario]) {
...
function atualizarTotalFormulario(): void {
  const total = linhasItens.reduce((soma, ref) => {
    const quantidade = Number(ref.quantidade.value) || 0
    const valorUnitario = Number(ref.valorUnitario.value) || 0
...
function validarFormulario(itens: Array<{ produto: string; quantidade: number; valorUnitario: number }>): string | null {
  ...
  for (const item of itens) {
    ...
    if (!Number.isFinite(item.valorUnitario) || item.valorUnitario < 0) {
      return 'O valor unitário de cada item não pode ser negativo.'
    }
  }
  return null
}
